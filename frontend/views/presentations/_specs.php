...
if($specs){
	echo '<label class="control-label col-md-3" for="presentations-imagefiles">Izabrane karakteristike</label>';
	...
	foreach($specs as $spec){
		...
		if($spcf_models = $spec->models){
			...
			echo '<td style="width:30%;">'.c($spec->spec->property->tName). '</td><td><b>'.$spec->value_operator.' ' .$spec->value .' ' . $spec->spec->property->unit->oznaka. '</b></td>';
		}
		echo '</tr>';		
	}
	echo '</table></div>';
